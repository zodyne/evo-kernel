...
**证据**（suc221 faf_offline progress_plot.py，2026-08-03）：点云 3D 图分全景 / 放大两级 panel，最初两级都报「贴合目标点数」，全景级按全场景统计、放大级按局部窗口统计，同名指标两个数。补丁注释记录决策：「全景 panel 只报总点数；贴合目标只在放大级报，免得两级同名指标数字对不上」，改后重出图通过目检。
