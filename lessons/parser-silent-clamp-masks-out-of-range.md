...
为什么：SUC221 `uv_to_xyz` 把 `u` 夹进 `[-cos_el, cos_el]`，导致全部出圆点（u²+v²>1）被夹成 |az|=90° 的"合法"假点——000028 数据集 3,559 个假点全部来自出圆点，且与真点无法区分，极易把解析缺陷误判成 FPGA 测角算错。改成出圆点返回 NaN + `off_disk` 标志后，三个数据集验证 NaN 与出圆标志逐点一致，且**圆内点与旧实现逐位相同**（回归无差）。
