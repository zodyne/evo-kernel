...
**为什么**：SUC221 libsuc221 移植核查会话中，`make CC=cc` 构建通过后，正是靠 `nm -gU libSPX_ALG.dylib | grep -iE "Faf|Fpga"` 确认 faf 9 个导出符号齐全、靠 `nm ... | grep Track` 排查 Track* 未定义符号，才把"能编译"和"模块真的进了库"分开验证。构建系统（尤其 GLOB 收集源文件）可能静默漏编整个目录，链接成 dylib 时不报错（macOS 动态库默认允许未定义符号）。
