...
证据（本session命令输出）：
- 修复前：`clangd --check=embedded/faf.c` → `Failed to find compilation database for .../embedded/faf.c`（同命令对 `libsuc221/src/.../ifAlg.c` 则正常 Loaded）；
- `cd embedded && make compdb` 生成 `compile_commands.json`；
- 修复后：`clangd --check=embedded/faf.c` → `Loaded compilation database`；`nvim --headless` 显示 `libsuc221 -> clangd | root: .../libsuc221`，`embedded -> clangd | root: .../embedded` 两子项目各自挂根成功。
