...
**为什么（SUC221 实例）**：`dc` 被当作直耦泄漏占比设 0.50 阈值硬剔除，实测它实际是**以 cal 方向为中心的角度陷波器**（量的是来波方向，不是泄漏占比）：0709_2 的真直耦点 dc 中位仅 0.426、无一超阈；而 000028 上 10–20 m 真目标 56.9% 超阈被误杀。根因是物理的：校准后泄漏的通道增益被 cal 抵消，剩下的分量反映方向而非泄漏。
