...
**为什么**：移植 false_alarm_filter 到 libsuc221 时，模块自带手写 Makefile 编译静态库，顶层 `Makefile` 规则将其静默排除出版本库——静默丢失构建入口比编译错误更难发现。

**边界**：写完 ignore 规则后用 `git check-ignore -v <path>` 验证关键源码路径不被误伤；否定规则要具体到路径，避免重新放开生成物。

**证据**：2026-07-31 libsuc221 会话 .gitignore diff，规则注释+否定放行落地。
