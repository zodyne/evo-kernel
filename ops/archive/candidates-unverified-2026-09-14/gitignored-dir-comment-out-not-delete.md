...
**为什么**：`libsuc221/` 这类嵌入式工作克隆常整体进 .gitignore，删掉即永久丢失，没有 git 可回退。会话中处理报告第 4 节时选择逐行注释（27 行）而非删除，旧 2D 绘图函数挂到 `--extra` 开关下而非移除。

**边界**：若目录实际有独立 git 仓库（工作克隆自身是 repo），则不受此限，删前先确认；长期无人用的死代码仍应定期清，注释里标注日期与原因。

**证据**：suc221-pointcloud-2.0 报告精简会话（2026-08-03），用户要求「移除第 4 节」，因无版本兜底改为注释并明确告知恢复方式。
