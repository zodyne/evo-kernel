...
**为什么**：复数除法与逐分量实数除法的舍入次序不同——乘倒数多一次 `1/s` 的舍入。SUC221 C 移植在**三处**命中此坑：cal 归一化、xn 归一化、A_grid 除 sqrt(6)。

**反例/边界**：只在要求与 numpy 参照逐点可比（验收/golden 比对）时才是坑；不比对参照的独立实现逐分量相除完全合法。

**证据**：SUC221 C 移植对齐 numpy 会话（capture-2026-07-27-14-36-51-742-ee3f），三处归一化按复数除法（乘倒数）改写后与参照对齐。
