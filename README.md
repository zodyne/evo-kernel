...
**现状一句话**：词法检索层的天花板已测清（recall 28–33%，调阈值最多 +7pp），
而 agent 语义通道（`evo candidates` + `evo get`）**模拟实测 precision 71% / recall 67%**。
**代码与告知都已就位，现在缺的只是「真实 agent 会不会用它」的证据。**

### 第一步：跑这三条

```bash
evo reflect     # 看 L3b agentic 通道 使用/精度 两行
evo doctor      # 自检，应 0 FAIL（WARN 允许存在：transcript 时效那条是已知的 C1 漂移）
npm test        # 应 FAIL=0（**不写死 PASS 数**：本库自己的 doc-selfreported-counts-drift
                #  条目就是讲这个坑 —— 文档里自述的计数漂移得比正文快得多）；
                #  当前基线的真值用 `evo doctor --full` 看，别信任何文档里的数字
```

### 判据表：看到什么 → 做什么

| `L3b agentic 通道使用` | 意味着 | 做什么 |
|---|---|---|
| `从未使用` / < 10 次不同任务 | 路由还没被真正走到 —— **不是失败，是数据不够** | 等，别改代码。若过了很久仍为 0，查 primer 是否还在两处装好（doctor 第 17 项） |
| ≥ 10 次不同任务 | 数据够了 | 读 `L3b agentic 通道精度`，与 `M1 召回精度` 对比 |

| `L3b agentic 通道精度` | 结论 |
|---|---|
| 明显高于召回精度（>10pp） | 形态 B 在真实使用中也成立 ⇒ **下一步：把默认路径从「自动注入」迁到「只给短名单」** |
| 与召回精度相当 | 真实 agent 挑得不如模拟（模拟是上界）⇒ 查它是没通读全量清单，还是挑得保守 |
| 低于召回精度 | 反而更差 ⇒ 先查 `agentic.jsonl` 里 `get` 的 ids 与 session 对应是否可靠 |

### 仪表（各自解决什么，别重造）

| 工具 | 回答什么 |
|---|---|
| `test/retrieval-bench/labeling/run-eval.js` | 任何改动之后：precision 与 recall **一起**报 |
| `test/retrieval-bench/labeling/FINDINGS.md` | 四轮测量的结论与局限（含 disputed 口径） |
| `test/retrieval-bench/replay.js` | 评分/后端变更的 §5.0 相对仲裁（559 条历史查询，丢失清单附精度） |
...
| 方向 | 为什么排除 |
|---|---|
| 继续调 `relevance` 阈值 | 已近最优：代价闭式解 0.225 与实测吻合，移动只买 **+7pp** recall |
| 换评分公式（v1 分母下限 / v2 idf / v3 叠加 / v4 最少命中数） | v1/v3 误伤短 trigger（短查询丢失 60%）；v2 只是缓解；v4 机制上无效（该路径是 2/8 不是 1/4） |
| 用「零模型显著性」替代固定比例 | 就是 **DFR**，且 TF-ICF ≈ Fisher 精确检验 p 值 ⇒ 与 idf 家族等价，**换皮** |
| 治「长 prompt 过度注入」 | 它是我的实现产物（字段长度归一 + 跨查询绝对阈值），**不是长查询的固有难题**；而代价只是 token（预算仅 3% 打满），真损失在召回侧 |
| 靠加闸门提精度 | 加闸门压的是召回，而**漏/误 = 3.44** ⇒ 亏。本会话加的两道已收窄一道 |

### 方法纪律（本会话血的教训）

1. **只测一侧必出错** —— 同一错误本会话犯三次（只报精度不报召回 / 拿历史注入比今天的库 /
   把空命中混两形态）。
2. **仪器第一次跑出「全 0/全空」时先怀疑环境** —— `run-eval.js` 根目录路径写错过一次，
   两数全 0 而输出看着正常。
3. **守护要查「值没被改动」，不是「能跑」** —— 两个静默故障（achan 作用域、reconciled 计数）
   都是能跑但数错。
4. **同一问题第二次调参无定论 → 查文献；按文献实施后再错 → 再查一次。**
   本会话三轮查证各自推翻了前一轮的方案。
5. **验证器自己需要被验证** —— doctor 第 13 项曾只查 `.gitignore` 的**文本**，
   而不查文件是否真被 ignore。
