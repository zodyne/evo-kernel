...
**现状一句话**：9/15 抽出的两个仪器故障（hermes 侧自污染反馈环、驱动器锁）已修并复验；
9/16 又补上**对账去重**（重试重写不再双计，实测重复 69/364）。**账目已干净，现在纯等数据**：
L3b agentic 通道 = 1 次 get + 1 次 candidates（精度 50%，n=2），离判据门槛（≥10 个任务）还早。
...
| 队列长度 | ~60 条（日登记 ~70） | 持续增长 ⇒ 产能不够（现 14:30 批次 `--max 8`）；不增长 ⇒ 收支平衡 |
