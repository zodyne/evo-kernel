...
反例/边界：与"嵌套仓库导致 git diff 空"（文件其实在 inner .git 里）和"gitignore 导致 diff 空"是同一症状（git diff/status 静默空）的三种不同根因，排查时要逐个排除，不能看到空输出就认定"改动丢失"。`ls-files --error-unmatch` 对被 .gitignore 排除的文件同样返回"未跟踪"，无法区分"从未 add"与"被 ignore"，还需再核对 .gitignore。另一形态是「已跟踪未提交」：修复只活在暂存区/工作区（`git status --short -- <file>` 显示 ` M`）时，`git log -- <file>` 翻不出它、也无法用历史回滚，必须先用 `git status` 判定「在不在提交里」，再决定走 `git log`/`git show` 还是只能读工作区 diff。
