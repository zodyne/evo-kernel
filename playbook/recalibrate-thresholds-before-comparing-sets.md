...
**为什么（SUC221 实例）**：persist 从 ±1 帧改成仅前帧（消除 1 帧延迟）。沿用旧 keep_score 测得 KEEP 暴跌 38.8%、IoU 61%——但 persist 占 0.35 权重，抽掉后整条 score 分布下移，旧阈值当然切掉更多；这测的是阈值失配。按同分位点把 keep_score 从 0.55 重标到 0.4597（KEEP 数量对齐）后，IoU 才是真实代价 47%。进一步用未参与 persist 计算的 SNR 与其他独立特征做成分对比：换进来的点 SNR 更高、hpr 更低——判定为**重心转移而非退化**。

**反例/边界**：变更不涉及打分/阈值链路（纯格式/性能改动）时无需重标定；重标定必须用"同分位点/同数量"这类中性锚点，不能用结果指标本身当锚（那是循环论证）。

**证据**：capture-2026-07-28-04-00-17-843-ewwj（SUC221 persist 变更评估全流程实测数据如上）。
