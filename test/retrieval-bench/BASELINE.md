...
### 2026-09-14（同日，第二次）· §5.0 回放仲裁 v0/v1/v2/v3/v4（**未 cutover**）

**起因**：long 组测出长 prompt 过度注入（`L-noise-schedule` 召回毫不相关的
`bash-loop-child-steals-stdin`）。诊断出机制后，各评分变体首次有了可仲裁的用例。

**机制（逐 term 复现，非推断）**：失败路径是 `trig` 通道。`trig = max over triggers of cover`，
而 `cover = 命中 / |phrase|` **按短语自身长度归一**——所以一个 8-bigram 的 trigger 只需命中
**2 个**就得到 `2/8 = 0.25`，**恰好等于阈值**。长 prompt（本例 104 个 term）里出现 2 个通用
bigram 纯属偶然。
`tag` 通道当年被同样的毛病逼着改成「取并集」，**`trig` 通道被漏下**，至今是 max。

**先补了仲裁工具**（此前 §5.0 回放是临时的、只抄了输出进本文件，无法复现同一口径）：
`test/retrieval-bench/replay.js` —— 在临时 ROOT 副本里回放 recall.jsonl 去重查询，
对比两个变体的注入集，并**给每条丢失附上对账精度**（把「逐条人审」里最可机械化的那一半自动做掉）。

**bench（绝对质量，含本轮新增的 long 组）**

| 变体 | learning | transfer | change | noise | long | 总噪声 |
|---|---|---|---|---|---|---|
| v0（默认）| 100% | 20% | 100% | 83% | 75%（top1 50%）| 10 |
| v1 | 100% | 20% | 100% | 100% | 75% | 7 |
| **v2** | 100% | 20% | 100% | 83% | **100%** | 6 |
| **v3** | 100% | 20% | 100% | 100% | **100%** | **3** |
| v4（本轮新增）| 100% | 20% | 100% | 100% | **75%** ❌ | 7 |

**§5.0 回放（相对对账，559 条去重查询）**

| 变体对 | 完全一致 | 丢失实例 | 长查询 丢失/保留 | 短查询 丢失/保留 |
|---|---|---|---|---|
| v0→v1 | 351 | 249 | 162 / 734 | **38 / 25** ❌ 短查询几乎砍光 |
| **v0→v2** | 370 | **299** | **287 / 612** | **3 / 60** ✅ |
| v0→v3 | 257 | 554 | 448 / 448（砍半）| 42 / 21 ❌ |

**丢失清单附精度后的判定**

| 变体 | 可审计的高精度丢失（样本 ≥3）|
|---|---|
| v2 | `verify-external-references` 6×（63%）· `stamp-baseline-before-review` 13×（100%）|
| v3 | `episode-suc221-project-overview` **105×（71%）** · `playbook-suc221-cfar-point-cloud-filtering` **40×（85%）** · `verify-external-references` 9×（63%）|

**结论**

1. **v1/v3 的「分母下限」会误伤短 trigger** —— 短查询丢失率 60%，这正是它们从未被采纳的原因，
   也解释了为何只在短查询组成的旧 bench 上「四变体无差别」：**旧 bench 测不出这个代价**。
2. **v2（idf 加权）是本轮最轻的有效修法**：bench 上 long 75%→100% 且其余阶段全等；
   回放丢失集中在长查询（287/612），短查询只丢 5%。它直接治「通用 bigram 偶然命中」，
   而 v3 的额外收益只有 bench noise 组的 1 个用例（`N-download`），代价是多丢 255 个实例。
3. **v4（要求最少命中 2 个 term）是负面结果**：实测该失败路径是 2/8 而非 1/4，
   「最少命中数」对长 trigger 无效 —— 记录在此以免日后重试。
4. **v2 的 cutover 未执行**：§5.0 要求「丢失清单逐条人审，任何未审回归不 cutover」，
   而 v2 仍有 2 条可审计的疑似回归（共 19 个实例）。**这两条应由人判**：
   `stamp-baseline-before-review`（13×，3/3）与 `verify-external-references`（6×，5/8）。
